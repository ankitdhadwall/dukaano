...
export default tseslint.config(...base, {
  rules: {
    ...
    /**
     * `consistent-type-imports` is OFF for the API, and this is a correctness requirement rather
     ...
     * and fails later.
     *
     * The rule's `--fix` cannot tell a DI token from a plain type, so leaving it enabled means an
     * autofix run can silently break the application. The rule stays ON for packages/*, which
     * have no decorators and no DI.
     */
    ...
    'no-restricted-syntax': [
      ...
      {
        // §13: raw SQL is permitted, but only via tagged templates, which Prisma parameterizes.
        // $queryRawUnsafe / $executeRawUnsafe concatenate strings and defeat that protection.
        selector:
          "CallExpression[callee.property.name=/^\\$(queryRawUnsafe|executeRawUnsafe)$/]",
        message:
          'Unsafe raw SQL is banned. Use the tagged-template forms ($queryRaw / $executeRaw) so ' +
          'Prisma parameterizes the query (blueprint §23.5).',
      },
    ],
  },
})
